...
<body>
    <?php
        ...
        if(isset($_POST['enviar'])){
            ...
            $query="UPDATE bebida SET nombre='$nombre', precio=$precio where id=$id ";
            $resultado=mysqli_query($Conexion,$query);
            if($resultado){
                echo"<script languaje='JavaScript'>alert('Los datos fueron ingresados correctamente');location.assign(Lista_Bebidas.php);</script>";
            }
            else{
                echo"<script languaje='JavaScript'>alert('Los datos NO FUERON ingresados correctamente');location.assign(Lista_Bebidas.php);</script>";
            }
...
<h1>Editar una bebida</h1>
    <form name="EditarBebida" method="post">
        <p><label>ID:</label>
